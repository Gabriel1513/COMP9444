...
def my_relu(in_value):
    ...
    """
    return tf.maximum(tf.constant(dtype=tf.float32,value=0),in_value)
...
def my_perceptron(x):
    ...
    w = tf.get_variable(name="weight",dtype=tf.float32,initializer=tf.ones(shape=(x)),trainable=True)


    y = tf.tensordot(i,w,1)
...
def onelayer(X, Y, layersize=10, input_size=784):
    """
    ...
    """
    w = tf.Variable(tf.zeros(shape=[input_size,layersize]),name='w')
    b = tf.Variable(tf.zeros(shape=[layersize]),name='b')
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    w1 = tf.Variable(tf.zeros(shape=[784,hiddensize]),name='w1')
    b1 = tf.Variable(tf.random_normal(shape=[hiddensize]),name='b1')

    first_layer_output = tf.matmul(X,w1) + b1
    first_layer_activation = tf.nn.relu(first_layer_output)

    w2 = tf.Variable(tf.zeros(shape=[hiddensize,outputsize]),name='w2')
    b2 = tf.Variable(tf.random_normal(shape=[outputsize]),name='b2')

    logits = tf.matmul(first_layer_activation,w2) + b2
    preds = tf.nn.softmax(logits)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    conv1 = tf.layers.conv2d(inputs=X, filters=convlayer_sizes[0], kernel_size=filter_shape[0], padding=padding, activation=tf.nn.relu)
    conv2 = tf.layers.conv2d(inputs=conv1, filters= convlayer_sizes[1], kernel_size=filter_shape[0], padding=padding, activation=tf.nn.relu)

    flattened_value = tf.reshape(conv2, [-1, int(conv2.shape[1])*int(conv2.shape[2])*int(conv2.shape[3])])
